    ...
    : std::integral_constant<bool,
        std::is_same<
            decltype(std::declval<RangeRule const&>().first(
                std::declval<char const*&>(),
                std::declval<char const*>())),
            system::result<typename RangeRule::value_type>>::value &&
        std::is_same<
            decltype(std::declval<RangeRule const&>().next(
                std::declval<char const*&>(),
                std::declval<char const*>())),
...
template<class RangeRule>
using is_range_rule = implementation_defined::is_range_rule<RangeRule>;
...
private:
    ...
    verify_range_rule() noexcept
    {
#ifdef BOOST_URL_HAS_CONCEPTS
        static_assert(
            ::boost::urls::grammar::RangeRule<any_rule>,
            "any_rule requirements not met");
#else
        static_assert(
            ::boost::urls::grammar::is_range_rule<any_rule>::value,
            "any_rule requirements not met");
#endif
    }
